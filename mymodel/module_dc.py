...
def generate_cross_modal_mask(ehr_cls_index=None, cxr_cls_index=None, note_cls_index=None, demo_cls_index=None, total_lens=None):    
    ...
    # mask[2, 2] = 0
    # mask[2, ehr_cls_index + 1: note_cls_index] = 0
    # mask[2, note_cls_index + 1: total_lens] = 0
...
class MoEGating(nn.Module):
    ...
    def forward(self, x):
        ...
        topk_weights, topk_indices = torch.topk(F.softmax(logits, dim=-1), self.k, dim=-1)
        return topk_weights, topk_indices  # [batch, k], [batch, k]

class TransformerMoE(nn.Module):
    def __init__(self, input_size, output_size, hidden_size, num_experts=6, k=4):
        ...
        self.experts = nn.ModuleList([Transformer(hidden_size, hidden_size*4)  for i in range(self.num_experts)])

        self.gating = MoEGating(output_size, num_experts, k)

        assert(self.k <= self.num_experts)

    def forward(self, x ,modal_mask, padding_mask=None):
        # x: [batch, seq_len, d_model] => need [seq_len, batch, d_model]
        x_t = x.transpose(0, 1)  # [seq_len, batch, d_model]
        batch_size = x.size(0)

        # Gating on the first token or mean-pooling
        gating_input = x[:, 0, :]  # [batch, d_model]
        topk_weights, topk_indices = self.gating(gating_input)  # [batch, k], [batch, k]

        # Initialize output
        output = torch.zeros_like(x_t)  # [seq_len, batch, d_model]

        for i in range(self.k):
            expert_idx = topk_indices[:, i]  # [batch]
            for eid in range(self.num_experts):
                selected = (expert_idx == eid)
                if selected.sum() == 0:
                    continue
                sel_idx = selected.nonzero(as_tuple=True)[0]
                sel_input = x_t[:, sel_idx, :]  # [seq_len, selected, d_model]
                sel_padding = padding_mask[sel_idx] if padding_mask is not None else None

                expert_out = self.experts[eid](sel_input, modal_mask, sel_padding)
                output[:, sel_idx, :] += topk_weights[sel_idx, i].view(1, -1, 1) * expert_out

        return output.transpose(0, 1)  # [batch, seq_len, d_model]

class Transformer(nn.Module):
    def __init__(self, hidden_dim, ff_dim, num_heads=2, num_layers=4):
    ...
    def forward(self, x, modal_mask, modal_pad):
        # Input shape: (seq_len, batch_size, hidden_dim)
        output = self.transformer(x, mask = modal_mask, src_key_padding_mask = modal_pad)
        return output
