...
from mymodel.design_choices import Design
...
def main():
    ...
    model = Design(hidden_dim=args.hidden_dim, layers=4, expert_k=2, expert_total=10, device=device).to(device)
    ...
    file_path = 'log/dc['+args.model+']_lr_' + str(args.lr) + '_seed_' + str(args.seed) + '_epoch_' + str(args.epochs) +'_' + str(args.task)+ '_' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '.txt'
    best_epoch = 0
    best_valid_res = 0
    best_test_auc = 0
    best_test_aupr = 0
