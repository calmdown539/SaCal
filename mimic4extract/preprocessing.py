...
def transform_ethnicity(ethnicity_series):
    ...
    return {'Race': ethnicity_series.fillna('').apply(lambda s: e_map[s] if s in e_map else e_map['OTHER'])}


def assemble_episodic_data(stays, diagnoses):
    data = {'Icustay': stays.stay_id,  'Length of Stay': stays.los,
            'Mortality': stays.mortality_inhospital, 'Readmission': stays.readmission} # 'Age': stays.age
    data.update(transform_gender(stays.gender))
    data.update(transform_ethnicity(stays.race))
    data['Height'] = np.nan
    data['Weight'] = np.nan
    data = DataFrame(data).set_index('Icustay')
    data = data[['Race', 'Gender',  'Height', 'Weight', 'Length of Stay', 'Mortality', 'Readmission']] # 'Age'
