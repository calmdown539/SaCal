...
from mimic4extract.util import dataframe_from_csv


def read_patients_table(path, mimic3=False):
    pats = pd.read_csv(path)
    if mimic3:
        pats = pats.rename(columns={'SUBJECT_ID':'subject_id','GENDER':'gender','DOD':'dod'})
        columns = ['subject_id', 'gender', 'dod']  
        print(pats.columns)
    else:
...
def read_admissions_table(path, mimic3=False):

    admits = pd.read_csv(path)
    if mimic3:
        admits = admits.rename(columns={'SUBJECT_ID':'subject_id','HADM_ID':'hadm_id',
    ...
    admits = admits[['subject_id', 'hadm_id', 'admittime', 'dischtime', 'deathtime', 'race', 'language', 'marital_status', 'insurance']]  
...
def read_icustays_table(path, mimic3=False):
    stays = pd.read_csv(path)
    if mimic3:
        stays = stays.rename(columns={'SUBJECT_ID':'subject_id','HADM_ID':'hadm_id','ICUSTAY_ID':'stay_id','FIRST_CAREUNIT':'first_careunit','LAST_CAREUNIT':'last_careunit',
        'INTIME':'intime','OUTTIME':'outtime','LOS':'los'})
...
def read_icd_diagnoses_table(path, mimic3=False):

    codes = pd.read_csv(f'{path}/d_icd_diagnoses.csv')
    diagnoses = pd.read_csv(f'{path}/diagnoses_icd.csv')

    if mimic3:
        codes = codes.rename(columns={'ICD9_CODE':'icd_code','LONG_TITLE':'long_title',})
        diagnoses= diagnoses.rename(columns={'ICD9_CODE':'icd_code','SUBJECT_ID':'subject_id','HADM_ID':'hadm_id','SEQ_NUM':'seq_num'})
...
def add_readmission_to_icustays(stays):
    dieinward = (stays.mortality == 1) & (stays.mortality_inhospital == 1) & (stays.mortality_inunit == 0)
    stays['dieinward'] = dieinward.astype(int)

    def next_admittime(group):
        group['next_admittime'] = group.admittime.shift(-1)
        return group

    new_stays = stays.sort_values('admittime').groupby(['subject_id'])
    new_stays = new_stays.apply(next_admittime)

    new_stays['diff'] = new_stays.next_admittime - new_stays.dischtime
    less_than_30days = new_stays['diff'].notnull() & (new_stays['diff'] < '30 days 00:00:00')
    new_stays['less_than_30days'] = less_than_30days.astype(int)

    new_stays.reset_index(level='subject_id', drop=True, inplace=True)

    new_stays['discharge_die'] = new_stays.dod - new_stays.dischtime

    new_stays['die_less_than_30days'] = (new_stays.mortality == 1) & (new_stays.mortality_inhospital == 0) & (
                new_stays.mortality_inunit == 0) & (new_stays.discharge_die < '30 days 00:00:00')
    new_stays['die_less_than_30days'] = new_stays['die_less_than_30days'].astype(int)

    new_stays['readmission'] = ((new_stays.dieinward == 1) | (new_stays.less_than_30days == 1) | (
                new_stays.die_less_than_30days == 1)).astype(int)

    stays = new_stays.drop(
        columns=['dieinward', 'next_admittime', 'diff', 'less_than_30days', 'discharge_die', 'die_less_than_30days'])

    stays.loc[(stays.mortality == 1) & (stays.mortality_inhospital == 1) & (stays.mortality_inunit == 1), 'readmission'] = 2
    stays = stays.sort_values('subject_id')
...
def read_events_table_and_break_up_by_subject(mimic3_path, table, output_path,
    ...
    for row, row_no, _ in tqdm(read_events_table_by_row(mimic3_path, table), total=nb_rows,
        ...
        row_out = {'subject_id': row['SUBJECT_ID'],
                   'hadm_id': row['HADM_ID'],
                   'stay_id': '' if 'ICUSTAY_ID' not in row else row['ICUSTAY_ID'],
