...
from mimic4extract.mimic3csv import *
from mimic4extract.preprocessing import add_hcup_ccs_2015_groups, make_phenotype_label_matrix
...
parser.add_argument('--test', dest='test', action='store_true', help='TEST MODE: process only 1000 subjects, 1000000 events.')
...
stays = add_readmission_to_icustays(stays)
...
for table in args.event_tables:
    read_events_table_and_break_up_by_subject(f'{args.mimic4_path}', table, args.output_path, items_to_keep=items_to_keep,
...
#####MIMIC-III#####
patients = read_patients_table(f'{args.mimic4_path}/hosp/PATIENTS.csv')
admits = read_admissions_table(f'{args.mimic4_path}/hosp/admission.csv')
stays = read_icustays_table(f'{args.mimic4_path}/icu/ICUSTAYS.csv')
