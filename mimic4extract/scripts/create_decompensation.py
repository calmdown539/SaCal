...
from mimic4extract.scripts.utils import merge_multimodal_data_without_cxr, create_train_val_test_set, random_sample
    ...
    for patient in tqdm(patients, desc='Iterating over patients'):
        ...
        for ts_filename in patient_ts_files:
            with open(os.path.join(patient_folder, ts_filename)) as tsfile:
                ...
                def judge_deathtime(deathtime):
                    try:
                        datetime.strptime(deathtime, "%Y-%m-%d %H:%M:%S")
                        return True
                    except ValueError:
                        return False
                if pd.isnull(deathtime) or judge_deathtime(deathtime)==False:
                    lived_time = 1e18
                else:
...
def main():
    ...
    merge_multimodal_data_without_cxr(args, 'decompensation')
