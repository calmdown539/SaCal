...
class IRENE(nn.Module):
    ...
    def forward(self, x, cc=None, ehr=None, sex=None, age=None, race=None, marital_status=None, insurance=None, labels = None, task= None):
        x, attn_weights = self.transformer(x, cc, ehr, sex, age, race, marital_status, insurance)
        x = self.ln(x.sum(dim=1))
        out = self.head(x)
        
        if torch.isnan(out).any() or torch.isinf(out).any():
                print("Out is NaN or Inf!")
        if task == "in-hospital-mortality":
            ...
            scores = torch.sigmoid(out)
        if task in ['length-of-stay',]:
            criterion_now = nn.CrossEntropyLoss()
            labels = labels.long().view(-1) if labels != None else labels
        else:
            #labels = labels.unsqueeze(1) if labels != None else labels
            criterion_now =  nn.BCEWithLogitsLoss()

        if labels is not None:
            loss = criterion_now(scores, labels)
            if torch.isnan(loss) or torch.isinf(loss):
                print("Loss is NaN or Inf!")
                loss = torch.nan_to_num(loss)
            return loss, scores, attn_weights
        else:
            return scores, attn_weights, out
