...
class FlexCare(nn.Module):
    ...
    def forward(self, ehr, ehr_lengths, use_ehr, demo, use_demo, note, use_note, task_index, labels, criterion):
        ...
        # demo_embed = self.demo_projection(demo)
        ...
        mm_mask[:, 3] = ehr_pad_mask[:, 0] | demo_pad_mask[:, 0] 
        mm_mask[:, 4] = cxr_pad_mask[:, 0] | note_pad_mask[:, 0]
        mm_mask[:, 5] = cxr_pad_mask[:, 0] | demo_pad_mask[:, 0]
        mm_mask[:, 6] = note_pad_mask[:, 0] | demo_pad_mask[:, 0]
        mm_mask[:, 7] = ehr_pad_mask[:, 0]
        mm_mask[:, 8] = cxr_pad_mask[:, 0]
