...
class FusionTrainer(Trainer):
    def __init__(self, 
        ...
        self.model = Fusion(args, self.ehr_model, self.cxr_model, self.device).to(self.device)
    ...
    def train_epoch(self):
        ...
        with tqdm(self.train_dl, position=0, ncols=150, colour='#666666',disable=False) as tqdm_range:
            for i, data in enumerate(tqdm_range):
                ...
                #     y = np.array(ys[:,1], dtype=np.float32)
                # elif self.args.task == 'length-of-stay':
                #     y = np.array(ys[:,2], dtype=np.float32)
                # elif self.args.task == 'readmission':
        ...
        ret = self.computeAUROC(outGT.data.cpu().numpy(), outPRED.data.cpu().numpy(),self.args.task, 'train')
    ...
    def validate(self, dl):
        ...
        with tqdm(self.val_dl, position=0, ncols=150, colour='#666666',disable=False) as tqdm_range:
            for i, data in enumerate(tqdm_range):
                x, seq_lengths, demo, ys = data
                x = torch.from_numpy(x).float().to(self.device)
                y = torch.from_numpy(ys).float().to(self.device)
    ...
    def train(self):
        ...
        for self.epoch in range(self.args.epochs):
            ...
            torch.cuda.empty_cache()
            print(f'Train epoch{self.epoch}:{train_ret}')
            
            # validation
            # self.model.eval()
            # val_ret = self.validate(self.val_dl)
            # torch.cuda.empty_cache()
            # print(f'Validation in epoch{self.epoch}:{val_ret}')
            # if self.best_auroc < next(iter(val_ret.values())):
            #     self.best_auroc = next(iter(val_ret.values()))
            #     self.best_stats = val_ret
            #     self.save_checkpoint()
                # print(f'saving best AUROC {ret["ave_auc_micro"]:0.4f} checkpoint')
                # self.print_and_write(train_ret, isbest=True)
            # else:
            #     self.print_and_write(train_ret, isbest=False)
            
        # Test
        with torch.no_grad():
            # model_path = f'{self.args.save_dir}/best_checkpoint.pth.tar'
            ...
            with tqdm(self.test_dl, position=0, ncols=150, colour='#666666',disable=False) as tqdm_range:
                for i, data in enumerate(tqdm_range):
                    x, seq_lengths, demo, ys = data
                    x = torch.from_numpy(x).float().to(self.device)
                    # if self.args.task == 'in-hospital-mortality':
            ...
            #print(f"val [{self.epoch:04d} / {self.args.epochs:04d}] validation loss: \t{epoch_loss/i:0.5f} \t{epoch_loss_align/i:0.5f}")
            ret = self.computeAUROC(outGT.data.cpu().numpy(), outPRED.data.cpu().numpy(), self.args.task, 'validation')
            print(f"Test results:{ret}")
