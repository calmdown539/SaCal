...
class Trainer():
    ...
    def computeAUROC(self, y_true, predictions, task, verbose=1):
        ...
        if task in ['in-hospital-mortality', 'decompensation', 'readmission']:
            ...
            for i in range(y_true.shape[1]):
                ...
                df = pd.DataFrame({'y_truth': y_true[:, i][label_mask], 'y_pred': predictions[:, i][label_mask]})
                
                (test_auprc, upper_auprc, lower_auprc), (test_auroc, upper_auroc, lower_auroc) = get_model_performance(df)
                test_auroc_ma = metrics.roc_auc_score(y_true[:,i][label_mask], predictions[:,i][label_mask], average='macro')
                auc_scores.append(test_auroc)
                auprc_scores.append(test_auprc)
                auc_scores_macro.append(test_auroc_ma)
            ...
            return { "auc_scores": auc_scores,    
                ...
                "auroc_ma" : auc_scores_macro,
                "auroc_ma_mean": np.mean(auc_scores_macro),
                'ci_auroc': ci_auroc,
                'ci_auprc': ci_auprc,
                }
            ...
            for i in range(y_true.shape[1]):
                label_mask = (y_true[:, i] > -1)
                test_auroc_mi = roc_auc_score(y_true[:,i][label_mask], predictions[:,i][label_mask], average='micro')
                test_auroc_ma = roc_auc_score(y_true[:,i][label_mask], predictions[:,i][label_mask], average='macro')
                test_auprc = metrics.average_precision_score(y_true[:, i][label_mask], predictions[:, i][label_mask])
                auc_scores_micro.append(test_auroc_mi)
                auc_scores_macro.append(test_auroc_ma)
                auprc_scores.append(test_auprc)
            auc_scores_micro = np.array(auc_scores_micro)
            auprc_scores = np.array(auprc_scores)
            auc_scores_macro = np.array(auc_scores_macro)
            return { "auc_scores_mi": auc_scores_micro,    
                "auroc_mi_mean": np.mean(auc_scores_micro),
                "auprc_mean": np.mean(auprc_scores),
                "auprc_scores": auprc_scores, 
                "auroc_ma" : auc_scores_macro,
                "auroc_ma_mean": np.mean(auc_scores_macro),
