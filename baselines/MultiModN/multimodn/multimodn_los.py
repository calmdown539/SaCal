...
def get_performance_metrics(y_true, y_pred, y_prob):
    f1_score_mi = F1Score(task="multiclass", num_classes=10, average='micro')
    f1_score_ma = F1Score(task="multiclass", num_classes=10, average='macro')
    roc = ROC(task="multiclass", num_classes=10)
    pr_curve = PrecisionRecallCurve(task="multiclass", num_classes=10)
    accuracy_score = Accuracy(task="multiclass", num_classes=10)
    roc_auc_score = AUROC(task="multiclass", num_classes=10, average='macro')
    confmat = ConfusionMatrix(task="multiclass", num_classes=10)
    auprc_metric = AveragePrecision(task="multiclass", num_classes=10, average='macro')

    auprc = auprc_metric(y_prob, y_true)
    ...
    return f1_score_mi(y_pred, y_true), f1_score_ma(y_pred, y_true), roc_auc_score(y_prob, y_true), auprc, accuracy_score(y_pred, y_true)#, sensitivity, specificity, fpr, tpr, precision, recall, \
    #  tn, fp, fn, tp, thr_roc, thr_pr
...
class MultiModN(nn.Module):
    ...
    def train_epoch(
        ...
        for batch in tqdm(train_loader):
            ...
            for dec_idx, decoder in enumerate(self.decoders):
                ...
                # prediction = [0.0 if i < 0.5 else 1.0 for i in output_decoder]
    ...
    def test(
        ...
        for dec_idx in range(len(output_decoder_epoch)):
            ...
            # prediction_epoch_dec_idx = [0.0 if i < 0.5 else 1.0 for i in output_decoder_epoch_dec_idx]
            # prediction_epoch_dec_idx = np.array(prediction_epoch_dec_idx)
            # prediction_epoch_dec_idx = torch.from_numpy(prediction_epoch_dec_idx)
            target_decoder_epoch_dec_idx = target_decoder_epoch[:, dec_idx]   
            results[dec_idx] = get_performance_metrics(target_decoder_epoch_dec_idx, prediction_epoch_dec_idx, output_decoder_epoch_dec_idx)
        return results     
