...
def get_performance_metrics(y_true, y_pred, y_prob):
    ...
    return roc_auc_score(y_prob, y_true), auprc, f1_score(y_prob, y_true), accuracy_score(y_pred, y_true)#, sensitivity, specificity, fpr, tpr, precision, recall, \
    #  tn, fp, fn, tp, thr_roc, thr_pr
    ...
    def train_epoch(
        ...
        for batch in tqdm(train_loader):
            ...
            for dec_idx, decoder in enumerate(self.decoders):
                ...
                output_decoder = decoder(state).squeeze()
                # print(output_decoder)
                # what, prediction = torch.max(output_decoder, dim=1)
                # print(what,prediction)
                # print(output_decoder)
