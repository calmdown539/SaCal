...
from MulTEHR.HGT_model import HGT
...
def main():
    ...
    model = HGT(device=device).to(device)
    ...
    file_path = 'log/hgt3_['+args.model+']_lr_' + str(args.lr) + '_seed_' + str(args.seed) + '_epoch_' + str(args.epochs) +'_' + str(args.task)+ '_' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '.txt'
    best_epoch = 0
    best_valid_res = 0
    best_test_auc = 0
    best_test_aupr = 0
    ...
    if not os.path.exists('hgtcheckpoints'):
        os.mkdir('hgtcheckpoints')

    if not os.path.exists('hgtlog'):
        os.mkdir('hgtlog')
    ...
    for epoch in tqdm(range(1, args.epochs+1)):
        ...
        for t in range(len(mutli_train_dl)):
            ...
            with tqdm(mutli_train_dl[t], position=0, ncols=150, colour='#666666',disable=False) as tqdm_range:
                for i, data in enumerate(tqdm_range):
                    ...
                    y_pred = model(ehr, ehr_length, mask_ehr, demo,mask_demo, task_index, y_true, criterion_now)
        ...
        # multitask_loss = mean + torch.nan_to_num(var, 0).item()
        ...
        with torch.no_grad():
            ...
            if valid_res > best_valid_res:
                ...
                torch.save(model.state_dict(), 'hgtcheckpoints/' + file_path[3:-4] + '.pt')
    ...
    # Test
    with torch.no_grad():
        model_path = 'hgtcheckpoints/' + str(file_path[3:-4]) + '.pt'
